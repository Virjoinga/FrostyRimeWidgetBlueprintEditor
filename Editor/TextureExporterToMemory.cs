...
namespace RimeWidgetBlueprintEditor.Editor
{
    ...
    public static class TextureExporterToMemory
    {
        ...
        public static void Export(Texture textureAsset)
        {
            ...
            TexturePlugin.ImageFormat format = TexturePlugin.ImageFormat.PNG;

            using (MemoryStream memoryStream = new MemoryStream())
            using (NativeWriter writer = new NativeWriter(memoryStream))
            {
                TexturePlugin.BlobData blob = new TexturePlugin.BlobData();
                TexturePlugin.FrostyTextureEditor.ConvertDDSToImage(ddsData, ddsData.Length, format, ref blob);
        ...
        private static byte[] WriteToDDS(Texture textureAsset)
        {
            ...
            switch (pixelFormat)
            {
                ...
                case "BC1A_UNORM": header.HasExtendedHeader = true; header.ExtendedHeader.dxgiFormat = SharpDX.DXGI.Format.BC1_UNorm; break;
                ...
                case "BC3_UNORM": header.HasExtendedHeader = true; header.ExtendedHeader.dxgiFormat = SharpDX.DXGI.Format.BC3_UNorm; break;
                case "BC3A_UNORM": header.HasExtendedHeader = true; header.ExtendedHeader.dxgiFormat = SharpDX.DXGI.Format.BC3_UNorm; break;
                ...
                case "BC5_UNORM": header.HasExtendedHeader = true; header.ExtendedHeader.dxgiFormat = SharpDX.DXGI.Format.BC5_UNorm; break;
